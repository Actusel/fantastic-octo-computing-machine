...
func _physics_process(_delta: float) -> void:
	...
	if player == null:
		# We still call move_and_slide to apply zero velocity
		move_and_slide()
		player_exists = false
	else:
	...
	if not ray_cast.is_colliding() and player_exists:
		...
		if global_position.distance_to(last_known_position) > arrival_tolerance:
