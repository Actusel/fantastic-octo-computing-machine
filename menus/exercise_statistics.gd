...
@onready var add_graph: Button = $VBoxContainer/CreateNew/add_graph
@onready var exercise_type: OptionButton = $VBoxContainer/CreateNew/exercise_type
@onready var new_graph: LineEdit = $VBoxContainer/CreateNew/new_graph
...
func _ready():
	...
	add_graph.pressed.connect(_on_add_graph_pressed)
...
func _on_add_graph_pressed():
	if new_graph.text == "":
		return
	Global.exercise_db.set(new_graph.text, exercise_type.get_item_text(exercise_type.get_selected_id()))
	option_button.add_item(new_graph.text)
	graph_data[new_graph.text] = []
	new_graph.text = ""
